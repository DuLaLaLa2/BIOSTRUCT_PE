...
from v2_struct_transformer import (
    MaskedStandardFocalLoss,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train v2 RoPE Transformer with fixed |i-j| sequence bias and simplified structural bias."
    )
    parser.add_argument("--train-data", default="495.pt")
    parser.add_argument("--test-data", default="117.pt")
    parser.add_argument("--save-model", default="v2_best_struct_transformer.pt")
    ...
    parser.add_argument("--seq-distance-scale", type=float, default=128.0)
    parser.add_argument("--seq-bias-scale", type=float, default=1.0)
...
def main() -> None:
    ...
    total_train = sum(seq.length for seq in train_sequences)
    pos_train = sum(int((seq.y > 0.5).sum().item()) for seq in train_sequences if seq.y is not None)
    ...
    print(f"Train residues: {total_train}")
    print(f"Train positive residues: {pos_train}")
    print(f"Train positive rate: {pos_train / max(total_train, 1):.4f}")
    ...
    model_config = checkpoint_model_config(
        ...
        seq_distance_scale=args.seq_distance_scale,
        seq_bias_scale=args.seq_bias_scale,
    ...
    criterion = MaskedStandardFocalLoss(
        ...
        if val_mcc > best_val_mcc:
            ...
            save_checkpoint(
                args.save_model,
                {
                    ...
                    "architecture": (
                        "v2 RoPE Transformer with fixed negative |i-j| sequence bias and "
                        "RBF/direction/contact C-alpha structural attention bias"
                    ),
                },
            )
        else:
