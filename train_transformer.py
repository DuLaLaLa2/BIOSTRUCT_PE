...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a residue-level protein Transformer on ESM2+ProtT5 features."
    )
    parser.add_argument("--train-data", default="protein_graph_datas495.pt")
    parser.add_argument("--test-data", default="protein_graph_datas117.pt")
    parser.add_argument("--save-model", default="best_transformer_model.pt")
...
def main() -> None:
    ...
    train_data = load_pyg_data(args.train_data, require_y=True)
    sequences = build_protein_sequences(train_data)
    ...
    criterion = MaskedFocalBCEWithLogitsLoss(
        ...
        if val_mcc > best_val_mcc:
            ...
            save_checkpoint(
                args.save_model,
                {
                    ...
                    "position_encoding": (
                        "tAPE absolute positional vectors used in TUPE decoupled "
                        "content-content and position-position attention scoring"
                    ),
                },
            )
        else:
    ...
    if args.test_data:
        ...
        test_data = load_pyg_data(args.test_data, require_y=True)
        test_sequences = build_protein_sequences(test_data)
