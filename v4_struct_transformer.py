...
class SplitConvolutionalRandomFourierRelativeBias(nn.Module):
    """
    Exact local relative bias plus SPE-style long-range bias.

    The local table is only used inside the local window.
    The global branch is only used outside that local window.
    """
    ...
    def _global_bias_table(
        self,
        max_abs_relative: int,
        device: torch.device,
        dtype: torch.dtype,
    ) -> torch.Tensor:
    ...
    def forward(self, positions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        local_mask = abs_rel <= self.local_relative_position
        global_mask = ~local_mask
        local_bias = local_bias.masked_fill(~local_mask.unsqueeze(-1), 0.0)
        global_bias = global_bias.masked_fill(~global_mask.unsqueeze(-1), 0.0)
        return local_bias.permute(0, 3, 1, 2), global_bias.permute(0, 3, 1, 2)
...
class StructuralGlobalGate(nn.Module):
    def __init__(
        self,
        feature_dim: int,
        num_heads: int,
        hidden_dim: int = 64,
        dropout: float = 0.1,
        initial_global_scale: float = 0.05,
    ):
        super().__init__()
        ...
        self.head_scale = nn.Parameter(torch.full((num_heads,), float(initial_global_scale)))

    def forward(self, features: torch.Tensor, pair_mask: torch.Tensor) -> torch.Tensor:
        gate = torch.sigmoid(self.gate_mlp(features)).permute(0, 3, 1, 2)
        scaled_gate = self.head_scale.view(1, -1, 1, 1) * gate
        return scaled_gate.masked_fill(~pair_mask.unsqueeze(1), 0.0)


class ContactGatedStructureAwareMultiheadAttention(nn.Module):
    def __init__(
        self,
        d_model: int,
        num_heads: int,
        ...
        self.seq_bias = SplitConvolutionalRandomFourierRelativeBias(
            num_heads=num_heads,
        ...
        self.global_gate = StructuralGlobalGate(
            feature_dim=self.pair_features.feature_dim,
            num_heads=num_heads,
    ...
    def forward(
        ...
        local_bias, global_bias = self.seq_bias(positions)
        pair_features, pair_mask = self.pair_features(coords, positions, mask)

        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(float(self.head_dim))
        scores = scores + local_bias.to(dtype=scores.dtype)
        scores = scores + (
            self.global_gate(pair_features, pair_mask).to(dtype=scores.dtype)
            * global_bias.to(dtype=scores.dtype)
        )
...
class ContactGatedEncoderLayer(nn.Module):
    def __init__(
        self,
        d_model: int,
        num_heads: int,
        ...
        super().__init__()
        self.self_attn = ContactGatedStructureAwareMultiheadAttention(
            d_model=d_model,
            num_heads=num_heads,
            dropout=dropout,
            local_relative_position=local_relative_position,
...
class V4ContactGatedTransformer(nn.Module):
    def __init__(
        self,
        input_dim: int,
        d_model: int = 256,
        ...
        self.config = {
            ...
            "initial_global_scale": float(initial_global_scale),
        }
...
def build_model_from_config(config: Dict[str, object]) -> V4ContactGatedTransformer:
    return V4ContactGatedTransformer(
        ...
        initial_global_scale=float(config.get("initial_global_scale", 0.05)),
    )
