...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run inference with a v2 structural-bias Transformer checkpoint."
    )
    parser.add_argument("--data", default="117.pt")
    parser.add_argument("--model", default="v2_best_struct_transformer.pt")
    parser.add_argument("--output", default="v2_prediction_results.npz")
...
def main() -> None:
    ...
    if not isinstance(checkpoint, dict) or "model_state_dict" not in checkpoint:
        raise ValueError("The model file must be a v2_train_transformer.py checkpoint.")
