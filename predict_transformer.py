...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run residue-level inference with the protein Transformer checkpoint."
    )
    parser.add_argument("--data", default="protein_graph_datas117.pt")
    parser.add_argument("--model", default="best_transformer_model.pt")
    parser.add_argument("--output", default="prediction_transformer_results.npz")
...
def main() -> None:
    ...
    if not isinstance(checkpoint, dict) or "model_state_dict" not in checkpoint:
        raise ValueError(
            "The model file must be a train_transformer.py checkpoint, "
            "not a raw GraphSAGE state_dict."
        )
    ...
    data = load_pyg_data(args.data, require_y=False)
    sequences = build_protein_sequences(data)
